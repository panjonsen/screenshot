...
public partial class EditingForm : Form
{
  private Bitmap originalImage;
    ...
    private TextBox textInputBox = null; // 动态输入框
    private SelectionForm parentForm; // 保存SelectionForm引用

    public EditingForm(Bitmap image, Point location, SelectionForm parent)
    {
        if (image == null)
        {
    ...
    private void InitializeComponents()
    {
        ...
        {
            toolbarForm.Close();
            if (parentForm != null) parentForm.Enabled = true; // 使用引用恢复
        };
    }
    ...
    private Rectangle GetTextBounds(AddText text)
    {
        ...
        {
            SizeF size = g.MeasureString(text.Text, text.Font);
